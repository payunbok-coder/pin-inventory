(function () {
  ...
  function notifySharedOrder(detail) {
    window.dispatchEvent(new CustomEvent("pin-order-local-change", { detail }));
  }
  ...
  function addProduct(productId) {
    ...
    notifySharedOrder({ type: "set", productId, quantity });
    refreshAll();
  ...
  function changeQuantity(productId, value) {
    ...
    if (!quantity) {
      ...
      notifySharedOrder({ type: "remove", productId });
  ...
  elements.orderList.addEventListener("click", (event) => {
    ...
    if (deleteButton) {
      ...
      notifySharedOrder({ type: "remove", productId: deleteButton.dataset.delete });
      refreshAll();
      showToast("Removed from order");
    } else if (stepButton) {
  ...
  window.PIN_APP = {
    ...
    getOrderState: () => store.toObject(),
    replaceOrderFromSync(nextItems) {
      store.replace(nextItems);
      refreshAll();
    },
    resetLocalOrder() {
      store.clear();
